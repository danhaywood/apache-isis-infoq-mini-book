...
In the diagram you'll see that there's one "loop": there are two routes from `ScheduledGymClass` to `GymInstructor`, either directly or by way of `GymClass`.
...
As it happens, the structure of that entityis quite similar to the `Person` entity in the `persons` module, so I'm going to do some renaming:
